...
// --- solo asignar valores a inputs si existen ---
...
function saveSettings() {
  if(bgColorInput && transparentBgCheck && solidColorsCheck) {
    settings = {
...
window.updateText = (i,value)=>{ options[i].text=value; saveOptions(); };
window.updateColor = (i,value)=>{ options[i].color=value; saveOptions(); };
window.removeOption = (i)=>{ options.splice(i,1); saveOptions(); renderOptions(); };

if(addOptionBtn) addOptionBtn.addEventListener("click", ()=>{
  options.push({text:"Nuevo", color:"#ffffff"});
  saveOptions(); renderOptions();
});
